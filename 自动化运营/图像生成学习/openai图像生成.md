OpenAI图像生成功能可通过Image API和Responses API实现，常用模型为gpt-image-1、DALL·E 2和DALL·E 3。以下是具体使用方法、注意事项和相关细节：

### 使用方法
- **生成图像**：可通过Image API的Generations端点或Responses API基于文本提示生成图像。可设置`n`参数生成多张图像，默认生成1张。例如使用Python代码，指定模型为`gpt-image-1`，输入提示词“Draw a 2D pixel art style sprite sheet of a tabby gray cat”即可生成相应图像，并通过Base64编码保存为本地文件。
- **编辑图像**：利用Image API的Edits端点或Responses API可编辑现有图像。可上传一张或多张图像作为参考生成新图像，也可通过上传图像和蒙版进行inpainting（修复）操作。如使用Python代码，将模型设为`gpt-image-1`，传入参考图像和提示词，即可生成编辑后的图像。
- **生成图像变体**：仅Image API支持此功能，且只有DALL·E 2模型可用。通过传入一张现有图像，即可生成其变体图像。
- **自定义输出**：可设置图像尺寸（如`1024x1024`等）、质量（`low`、`medium`等）、格式（`png`、`jpeg`等）、压缩率（0-100%）和背景（透明或不透明）等。例如，若要生成透明背景图像，可将`background`参数设为`transparent`，且该功能仅在`png`和`webp`格式下支持。

### 注意事项
- **模型限制**：复杂提示可能需长达2分钟处理。文本渲染方面，虽有改进但仍可能存在文字 placement 和 clarity 问题。生成图像时，模型可能难以完全保持视觉一致性，在处理结构化或对布局敏感的构图时，可能无法精准放置元素。
- **内容审核**：所有提示和生成图像均需遵循OpenAI内容政策。使用`gpt-image-1`时，可通过`moderation`参数控制审核严格程度，`auto`为标准过滤，`low`为较宽松过滤。
- **成本与延迟**：图像生成成本由输入文本token、输入图像token（若编辑图像）和输出图像token构成。延迟与token数量成正比，图像尺寸越大、质量越高，token消耗越多，生成速度越慢。例如`gpt-image-1`生成1024×1024的高质量图像，需消耗4160个token。

### 相关细节
- **模型选择细节**：`gpt-image-1`是较新的多模态模型，指令遵循性强，文本渲染和细节编辑能力优。DALL·E 2成本较低，支持图像变体生成和inpainting。DALL·E 3图像质量更高，支持更大分辨率，但仅支持Image API的Generations端点。
- **多轮生成细节**：Responses API支持多轮图像生成，可通过`previous_response_id`参数或传入图像ID，结合新提示词迭代优化图像。例如先生成“灰色虎斑猫拥抱水獭”的图像，再传入该图像相关信息，输入提示词“Now make it look realistic”，即可生成更逼真的图像。
- **流式生成细节**：Image API和Responses API均支持流式生成，可通过设置`stream=true`和`partial_images`参数（取值0-3）获取部分图像，提升交互体验，但每张部分图像会额外消耗100个image output tokens。
- **蒙版编辑细节**：进行inpainting时，蒙版图像需与要编辑的图像格式、尺寸相同，且小于50MB，还需包含alpha通道。可使用Python的PIL库为黑白蒙版添加alpha通道。
